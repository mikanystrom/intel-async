...
\begin{equation}
  ...
  \label{eq:t50}
\end{equation}
where $D_0$ is in micrometres and the default
parameters are:
\begin{equation*}
...
\begin{equation}
  ...
  \label{eq:drex}
\end{equation}
where $D_0$ is the prior austenite grain size in micrometres.  During partial
...
\begin{equation}
  ...
  \label{eq:hp}
\end{equation}
where $D_\alpha$ is in millimetres.
...
The model is implemented in Modula-3 as two packages within the \texttt{m3utils}
tree:
...
The time integration uses explicit Euler with a default timestep of
\SI{0.01}{\second}.  All internal calculations use SI units (metres, Kelvin,
seconds, Pascals) with conversions to engineering units (micrometres,
millimetres) applied at formula boundaries where the empirical correlations
...
would be achieved in practice: the sub-nanometre precipitate sizes approach the
...
prediction, and superposition yield stress modelling.
