#!/bin/sh
#
# Sweep: maximize TM-30 Rf as a function of required efficacy
...
# For each threshold, finds the spectrum maximizing TM-30 Rf (color
# fidelity over 99 CES samples) while maintaining CRI >= $CRI_MIN
# at 2700K with Philips-corrected efficacy.
#
# Usage:
#   ./run-maxRf-sweep-parallel.sh          # CRI >= 80 (default)
#   ./run-maxRf-sweep-parallel.sh 95       # CRI >= 95
#
...
echo "=== Max-Rf sweep (Philips-corrected): CCT=${CCT} CRI>=${CRI} ==="
...
for eff in $EFFS; do
    ...
    $PHOTOPIC -grid -tm30 -run-maxRf-philips $CCT $CRI $eff -scm -scmfile $SCMFILE > /dev/null 2>&1 &
    running=$((running + 1))
...
echo "Generate figures with: python3 plot_maxRf_sweep.py ${CRI} --philips"
