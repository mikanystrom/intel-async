...
\subsection{TM-30 Color Fidelity and Gamut}
\label{sec:tm30-math}

IES~TM-30 (standardized as CIE~224:2017) addresses the well-known
deficiencies of CRI by using 99~Color Evaluation Samples (CES),
the CIECAM02 color appearance model, and the CAM02-UCS uniform color
space.  \photopic{} implements TM-30 in grid mode, enabled by the
\cmd{-tm30} flag.

\paragraph{Reference illuminant.}
CIE~2017 selects the reference illuminant based on CCT:
\begin{itemize}
  \item $\text{CCT} < 4000\,\text{K}$: Planckian blackbody $B(\lambda, T)$
  \item $4000 \leq \text{CCT} \leq 5000\,\text{K}$: linear blend of
    Y-normalized Planckian and CIE D-illuminant, with mixing fraction
    $m = (\text{CCT} - 4000)/1000$
  \item $\text{CCT} > 5000\,\text{K}$: CIE D-series illuminant
    $S_D(\lambda) = S_0(\lambda) + M_1 S_1(\lambda) + M_2 S_2(\lambda)$
\end{itemize}
where $S_0$, $S_1$, $S_2$ are CIE basis functions and $M_1$, $M_2$
are coefficients derived from the daylight-locus chromaticity at the
given CCT.

\paragraph{Color appearance.}
For each of the 99~CES, \photopic{} computes the reflected tristimulus
values under both the test and reference illuminants using the CIE 1964
$10^\circ$ standard observer.  The XYZ values are then transformed through
the CIECAM02 forward model (sharpened chromatic adaptation via $M_{\text{CAT02}}$,
nonlinear cone response compression, opponent color channels) to obtain
lightness~$J$, colorfulness~$M$, and hue angle~$h$.  These are then
mapped to the CAM02-UCS uniform space via:
\begin{equation}
...
\paragraph{TM-30 metrics.}
Three metrics are computed:
\begin{itemize}
  \item $R_f$ (Fidelity Index): the average color difference $\overline{\Delta E'}$
    in $J'a'b'$ space across all 99~CES, mapped to a 0--100 scale via
    a soft-capped formula:
    \begin{equation}
      R_f = 10 \ln\!\Bigl(\exp\!\bigl(\tfrac{100 - 6.73\,\overline{\Delta E'}}{10}\bigr) + 1\Bigr)
      \label{eq:Rf}
    \end{equation}
  \item $R_g$ (Gamut Index): the ratio of the test-illuminant gamut
    area to the reference gamut area in $a'b'$ space, computed as
    $R_g = 100 \times A_{\text{test}} / A_{\text{ref}}$ where areas are
    determined by the shoelace formula over 16~hue-bin centroids.
    $R_g > 100$ indicates gamut expansion, $R_g < 100$ indicates
    desaturation.
  \item $R_{cs,h_j}$ ($j = 1 \ldots 16$): chroma shift per hue bin,
    quantifying the direction of gamut change.
\end{itemize}
...
\subsection{Philips phosphor-converted LED model}
\label{sec:philips}

Real white LEDs produce light by pumping a short-wavelength (blue) LED
and converting part of the output to longer wavelengths via phosphor
down-conversion.  This process incurs two losses:
\begin{enumerate}
  \item \textbf{Stokes loss.}  A photon at wavelength $\lambda > \lambda_{\text{pump}}$
    carries less energy than the pump photon that created it.  The energy
    ratio is $\lambda / \lambda_{\text{pump}} > 1$.
  \item \textbf{Quantum efficiency.}  Not every pump photon produces an
    output photon; the average quantum efficiency is $\eta_{\text{QE}} < 1$.
\end{enumerate}

\noindent
Following the model used by Philips Research, we define the
\emph{cost function}:
\begin{equation}
\fboxsep=8pt
\boxed{\;
  C(\lambda) = \max\!\biggl(\, 1, \;\;
    \frac{\lambda}{\lambda_{\text{pump}} \cdot \eta_{\text{QE}}} \,\biggr)
\;}
  \label{eq:philips-cost}
\end{equation}
where $\lambda_{\text{pump}} = 450\,\text{nm}$ is the blue pump
wavelength and $\eta_{\text{QE}} = 0.90$ is the average phosphor
quantum efficiency.  $C(\lambda)$ gives the watts of electrical
(pump) power needed per watt of optical output at wavelength~$\lambda$.

The \emph{Philips-corrected luminous efficacy} replaces the simple
power integral in the denominator of~\eqref{eq:ler} with the
cost-weighted integral:
\begin{equation}
\fboxsep=8pt
\boxed{\;
  \eta_{\text{P}} = K_m \,
    \frac{\displaystyle\int_{\lambda_0}^{\lambda_1} S(\lambda)\,\bar{y}(\lambda)\,d\lambda}
         {\displaystyle\int_{\lambda_0}^{\lambda_1} S(\lambda)\,C(\lambda)\,d\lambda}
\;}
  \label{eq:philips-ler}
\end{equation}
This penalizes spectra with strong red content, since red photons at
$\lambda \approx 650\,\text{nm}$ cost $C(650) = 650/(450 \times 0.90)
\approx 1.60$ times as much pump power as blue photons.

\medskip\noindent
The Philips-corrected inverse mode (\cmd{-run-maxr9-philips}) solves:
\begin{equation}
\fboxsep=8pt
\boxed{\;
  \max_{\mathbf{p}} \; R_9\bigl(S(\lambda; \mathbf{p})\bigr)
  ...
  \begin{cases}
    R_a \geq R_a^{\min} \\[3pt]
    \min_{i \in 1..8} R_i \geq R_a^{\min} - 10 \\[3pt]
    \eta_{\text{P}} \geq \eta_{\min} \\[3pt]
    |\text{CCT} - T_{\text{target}}| \leq 50\,\text{K} \\[3pt]
    D_{uv} \leq 0.012
  \end{cases}
\;}
  \label{eq:inverse-philips}
\end{equation}
Note that $\eta_{\text{P}}$ is computed fresh at each optimizer
iteration from the current spectrum~$S(\lambda;\mathbf{p})$, so the
Philips loss model is fully integrated into the optimization loop, not
applied as a post-hoc correction.
...
\bibitem{cie2017}
CIE,
\emph{CIE 224:2017 --- Colour Fidelity Index for accurate scientific
use}, Commission Internationale de l'\'Eclairage, 2017.

\bibitem{li2002}
C.~Li, M.\,R.~Luo, B.~Rigg, and R.\,W.\,G.~Hunt,
``CMC 2000 chromatic adaptation transform: CMCCAT2000,''
\emph{Color Res.\ Appl.}, vol.~27, no.~1, pp.~49--58, 2002.
...
\subsection{\texttt{-run-maxr9-philips}: Maximize \texorpdfstring{$R_9$}{R9} with Philips correction}

\begin{lstlisting}[style=shell]
photopic -run-maxr9-philips <cct> <min-cri> <min-efficacy> \
         -scm -scmfile photopic.scm
\end{lstlisting}

Same as \cmd{-run-maxr9} but uses the Philips phosphor-converted LED
loss model (Section~\ref{sec:philips}) in the efficacy constraint.
The efficacy floor is interpreted as Philips-corrected lm/W, which
accounts for Stokes shift and quantum efficiency losses.  Typical
Philips-corrected values are 30--40\% lower than uncorrected LER.

Output files use the prefix \cmd{maxR9P} instead of \cmd{maxR9}.
...
\subsection{\texttt{-grid}: Grid-based integration}

\begin{lstlisting}[style=shell]
photopic -grid <other flags...> -scm -scmfile photopic.scm
\end{lstlisting}

Enables the grid-based integration mode, which pre-tabulates all
spectral weight vectors (color matching functions, test color sample
reflectances, photopic response) on a 391-point grid at 1\,nm
resolution.  Spectrum evaluation then reduces to dot products,
eliminating per-evaluation numerical integration.  Grid mode is
required for TM-30 computation and is recommended for all use.

\subsection{\texttt{-tm30}: TM-30 color metrics}
\label{sec:tm30-cli}

\begin{lstlisting}[style=shell]
photopic -grid -tm30 <other flags...> -scm -scmfile photopic.scm
\end{lstlisting}

Enables computation of TM-30 metrics ($R_f$, $R_g$, $R_{cs,h_j}$)
alongside CRI.  Requires \cmd{-grid} mode.  This flag loads the 99~CES
reflectance spectra (\file{ces99.scm}), the CIE 1964 $10^\circ$ color
matching functions, and the CIE D-illuminant basis functions at startup.

When \cmd{-tm30} is active, \cmd{grid-calc-specs} appends TM-30 data to
its return value (see Section~\ref{sec:tm30-access}).  TM-30 adds
approximately 30\% overhead to each spectrum evaluation due to the 99
additional color-sample dot products and CIECAM02 transforms.
...
The script \file{run-maxr9-sweep-parallel.sh} sweeps Philips-corrected
efficacy from 200 to 320\,lm/W, maximizing $R_9$ at each level using
...
The sweep uses the Philips phosphor-converted LED loss model
(Section~\ref{sec:philips}) so that the efficacy constraint reflects
real-world LED physics.  Each run produces \file{maxR9P} result files.

\subsection{Example 7: Single Philips-corrected run}

\begin{lstlisting}[style=shell]
$PHOTOPIC -run-maxr9-philips 2700 80 270 -scm -scmfile $SCMFILE
\end{lstlisting}

Maximizes $R_9$ at CRI $\geq$ 80 with Philips-corrected efficacy
$\geq$ 270\,lm/W.  This is equivalent to roughly 400\,lm/W uncorrected
LER.  The Philips model penalizes red spectral content since long-wavelength
photons require more pump energy to produce via phosphor conversion.

\subsection{Example 8: Plotting results}

After running optimizations, generate publication-quality figures:

\begin{lstlisting}[style=shell]
python3 plot_spectra.py                # forward problem spectra
python3 plot_maxr9_sweep.py --philips  # Philips-corrected R9 frontier
python3 plot_maxr9_sweep.py 95 --philips  # CRI >= 95
\end{lstlisting}
...
\begin{itemize}
  ...
  \item $R_9$ vs.\ efficacy Pareto frontier (uncorrected and Philips-corrected)
  \item Individual spectrum plots at each efficacy level
\end{itemize}
...
%======================================================================
\section{TM-30 Usage}
\label{sec:tm30-access}

TM-30 metrics are available in both interactive and batch modes when
\cmd{-grid -tm30} flags are active.

\subsection{Interactive TM-30 evaluation}

\begin{lstlisting}[style=scheme]
;; Compute TM-30 for a fluorescent illuminant
(let ((fl2-grid (spectrum->grid (FL 2))))
  (grid-calc-tm30 fl2-grid))
;; => (Rf Rg #(Rcs-h1 ... Rcs-h16) ((CCT Duv) (R1 ... R14)))

;; TM-30 for a blackbody (should give Rf~100, Rg~100)
(grid-calc-tm30 (grid-blackbody 3000))

;; Compute the CIE D-illuminant reference for a given CCT
(grid-d-illuminant 6500)     ;; D65-like spectrum
(grid-tm30-reference 4500)   ;; Planckian/D blend at 4500K
\end{lstlisting}

\subsection{TM-30 in \texttt{grid-calc-specs}}

When \cmd{*compute-tm30*} is true (set automatically by the \cmd{-tm30}
flag), \cmd{grid-calc-specs} appends a TM-30 data element to its
return list:

\begin{lstlisting}[style=scheme]
(cri-ra worst-ri efficacy
 ((cct duv) (R1 R2 ... R14))
 cri-14 worst-14 sel-cri sel-worst worst-axis
 ;; appended when -tm30 is active:
 (Rf Rg #(Rcs-h1 ... Rcs-h16) ((ref-cct ref-duv) ...)))
\end{lstlisting}

To extract TM-30 fields from the result:

\begin{lstlisting}[style=scheme]
(define s (grid-calc-specs my-sgrid))
;; TM-30 data is the last element (10th, 0-indexed 9th)
(define tm (list-ref s 9))     ;; TM-30 data list
(car tm)                       ;; Rf (fidelity index)
(cadr tm)                      ;; Rg (gamut index)
(caddr tm)                     ;; Rcs vector (16 hue bins)
(vector-ref (caddr tm) 0)      ;; Rcs for hue bin 1
\end{lstlisting}

\subsection{Validation}

A validation script is provided at \file{report-runs/validate-tm30.scm}
that compares \photopic{} TM-30 results against reference values from
the Python \texttt{colour-science} library:

\begin{lstlisting}[style=shell]
$PHOTOPIC -scm -scmfile $SCMFILE -grid -tm30 \
    -scmfile ../report-runs/validate-tm30.scm
\end{lstlisting}

Expected: Planckian self-tests give $R_f \approx 100$, $R_g \approx 100$;
FL2 gives $R_f \approx 70$, $R_g \approx 86$; FL11 gives
$R_f \approx 80$, $R_g \approx 100$.  All results agree with
\texttt{colour-science} within $\pm 2$ units.
...
\cmd{*pump-nm*}         & $450 \times 10^{-9}$ & Blue LED pump wavelength (m) \\
\cmd{*pump-qe*}         & 0.90  & Phosphor quantum efficiency \\
...
\cmd{*use-grid*}        & \cmd{\#f} & Grid-based integration (set by \cmd{-grid}) \\
\cmd{*compute-tm30*}    & \cmd{\#f} & TM-30 computation (set by \cmd{-tm30}) \\
\bottomrule
\end{tabular}
\end{center}
...
\file{src/ces99.scm}              & 99 CES reflectance spectra, 10$^\circ$ CMFs, D-basis \\
