...
\author{Mika Nystrom}
\date{March 7, 2026 \quad 10:30 PST}
...
inner loop from 160 iterations to $\sim$10, a potential
\textbf{10--20$\times$ speedup} on the state construction phase.
...
\midrule
1 & K1: Relevant symbols only & High   & Low      & 10--20$\times$ \\
...
The pragmatic path is to implement K1 (relevant symbols only)
first---it is a small, localized change to \texttt{PDA.m3} that
should deliver an immediate 10--20$\times$ speedup.  Grammar
refactoring (G1) can proceed in parallel.  Together, these should
bring the SV grammar build time down to a tolerable 5--15 seconds.

The ideal long-term fix is K3 (LALR(1) core merging), which would
