...
static void adc() {
    ...
    if (status & FLAG_DECIMAL) {
        uint16_t al, ah;
        al = (a & 0x0F) + (value & 0x0F) + (uint16_t)(status & FLAG_CARRY);
        if (al > 9) al += 6;
        ah = (a >> 4) + (value >> 4) + (al > 0x0F ? 1 : 0);
        /* Z flag is set from binary result */
        result = (uint16_t)a + value + (uint16_t)(status & FLAG_CARRY);
        zerocalc(result);
        /* N and V from intermediate high nibble */
        if (ah & 0x08) setsign(); else clearsign();
        overflowcalc(((ah << 4) | (al & 0x0F)), a, value);
        if (ah > 9) ah += 6;
        if (ah > 0x0F) setcarry(); else clearcarry();
        saveaccum((uint16_t)((ah << 4) | (al & 0x0F)));
        clockticks6502++;
    } else
    #endif
    {
...
static void sbc() {
    ...
    /* value is already XORed with 0xFF (one's complement of operand) */
    #ifndef NES_CPU
    if (status & FLAG_DECIMAL) {
        /* BCD SBC: documented behavior only.
         * Flags come from binary result, BCD result from decimal correction.
         * Algorithm from http://www.6502.org/tutorials/decimal_mode.html */
        uint8_t borrow = (status & FLAG_CARRY) ? 0 : 1;
        uint8_t orig = value ^ 0xFF;
        int al = (a & 0x0F) - (orig & 0x0F) - borrow;
        int s = (int)a - (int)orig - (int)borrow;
        if (al < 0) al -= 6;
        int ah = (a >> 4) - (orig >> 4) - (al < 0 ? 1 : 0);
        /* Binary result for flags */
        result = (uint16_t)a + value + (uint16_t)(status & FLAG_CARRY);
        zerocalc(result);
        signcalc(result);
        overflowcalc(result, a, value);
        if (s >= 0) setcarry(); else clearcarry();
        if (ah < 0) ah -= 6;
        saveaccum((uint16_t)(((ah << 4) & 0xF0) | (al & 0x0F)));
        clockticks6502++;
    } else
    #endif
    {
