...
that exhaustively checks for deadlock.  FDR3~\cite{fdr4}, the
...
The Fulcrum CSP dialect is a hardware-oriented
...
\begin{description}
\item[Hardware-targeted types.]
  Three integer types: \texttt{int} (arbitrary precision, the
  mathematical~$\mathbb{Z}$), \texttt{int(\nonterminal{W})} ($W$-bit
  unsigned), and \texttt{sint(\nonterminal{W})} ($W$-bit signed two's
  complement).  Expressions use arbitrary-precision arithmetic
  regardless of operand types; fixed-width variables truncate on
  assignment.  The language also supports \texttt{bool}, \texttt{string},
  arrays, and packed structures.

\item[Directional channels.]
  Channel operations are \texttt{L?x} (receive) and \texttt{R!x}
  (send).  Each channel has a statically declared width (\texttt{int(W)})
  and is used by exactly two processes---one sender, one receiver.
  The probe \texttt{\#L} tests whether a channel is ready (works on
  both input and output ports).

\item[Node ports.]
  In addition to channels, processes may communicate via \emph{nodes}:
  shared boolean variables accessible by multiple processes.  Nodes
  are used for low-level signaling and hardware protocols.  The
  protocol type framework proposed in this report applies to
  channel-based communication; node-based interactions require
  separate treatment.

\item[Deterministic selection.]
  The guarded command with \texttt{[\,]} separator selects among
  boolean guards; the programmer guarantees that at most one guard is
  true at any time:
\begin{Verbatim}[samepage=true]
*[[ #L -> L?x ; R!x
  [] #S -> S?y ; T!y
...
\item[Nondeterministic selection.]
  When the guard separator is \texttt{:} instead of \texttt{[\,]},
  the selection is nondeterministic: multiple guards may be
  simultaneously true, and the implementation chooses arbitrarily.
  The \keyword{else} keyword may appear as the last arm, providing a
  non-blocking fallback when no other guard is true.

\item[Parallel composition.]
  Processes run in parallel (inter-process, via the \texttt{.sys}
  topology).  Within a single process, the comma operator
  (\texttt{S1\,,\,S2}) provides intra-process parallel composition,
  executing \texttt{S1} and \texttt{S2} in an arbitrary interleaving.
...
\item[\texttt{csp2verilog} (BD synthesis).]
  ...
  SystemVerilog with bundled-data handshake protocols.  Supports
  coverage probes and state covergroups.

\item[\texttt{Cast2Verilog} (full design synthesis).]
  Comprehensive Java tool that handles both structural (PRS-based) and
  behavioral (CSP-based) cell designs via CAST, a wrapper format
  around Fulcrum CSP (the \texttt{.sys} format is an alternative).
  Generates hierarchical Verilog, testbenches, and co-simulation
  interfaces.
...
channel values (\texttt{int(W)}), eliminating delegation and
polymorphism; (3)~multi-channel
...
from \texttt{int} (the language's default arbitrary-precision type) to
\texttt{int(\nonterminal{W})} (fixed-width, required for synthesis).  The tool
...
\paragraph{Repeated channel access.}
The simple buffer insertion above assumes each channel is accessed at
most once per loop iteration.  When a process accesses the same
channel multiple times per iteration (e.g., two reads from a shared
function unit), straightforward buffer insertion does not apply.
Wen, Li, and Manohar~\cite{wen2025} address this by introducing
\emph{channel replicas}---one per access site---and a
structurally-inductive control circuit that dynamically routes tokens
between the replicas and the original channel.  Because the
construction uses only deterministic dataflow elements, the result is
guaranteed slack elastic and can be verified by the same protocol
...
channels~\cite{manohar2001}.  Slack elasticity is critical for the
verified refinement framework because it means that slack insertion
and removal are \emph{semantics-preserving transformations}---the
designer (or the \texttt{slacker} tool) can adjust buffering for
performance without affecting correctness.

Papadantonakis~\cite{papadantonakis2002} provides the most thorough
analysis of slack elasticity and its limitations.  He defines
\emph{D-CHP} (Deterministic CHP), a restricted subset of CHP that
excludes probes and the comma operator (undirected parallel
composition), and establishes a hierarchy of determinism conditions:
\begin{description}
\item[L-deterministic:] The local state of each process determines
  its next action (direction and channel) without consulting the
  environment.
\item[SS-deterministic:] The system's behavior is deterministic at
  the \emph{system state} level (the global state determines all
  future behavior).
\item[IO-deterministic:] The system maps input sequences to output
  sequences deterministically.
\end{description}
These conditions form a strict hierarchy:
L-deterministic $\implies$ SS-deterministic $\implies$ IO-deterministic,
with none of the reverse implications holding.

The key theorem is: \emph{every L-deterministic system is slack elastic.}
The proof works because, in an L-deterministic system, each process
knows its next action from its local state alone, so additional
buffering on channels cannot change the sequence of values produced.
However, SS-determinism---despite implying deterministic
behavior---is \emph{not} sufficient for slack elasticity.
Papadantonakis gives a counterexample: a system of two processes with
a cycle where the order of operations depends on the relative
scheduling, which in turn depends on buffer depths.

\paragraph{Beyond slack elasticity: value sequence systems.}
The most significant finding of~\cite{papadantonakis2002} is that
slack elasticity alone is \emph{insufficient} for the design
transformations that matter in practice.  Slack elasticity preserves
functional behavior when \emph{only slack changes}, but process
decomposition (splitting one process into two communicating
sub-processes) is not a slack change---it introduces new processes
and channels.  A system can be slack elastic yet decomposing one
of its processes may produce a system that is \emph{not} equivalent
to the original.

To address this, Papadantonakis introduces \emph{value sequence
systems}: an abstract framework where each channel carries a
deterministic sequence of values, and the system's behavior is
fully characterized by these sequences.  He then defines
\emph{domain weakening}---relaxing the domain of a value-sequence
function while preserving its behavior on the original domain---as
the formal justification for decomposition transformations.  A
process $P$ can be decomposed into $P_1 \| P_2$ with new internal
channels if the resulting system is a domain weakening of the
original value-sequence system.

This framework is directly relevant to our protocol types.  Our
protocol annotations on channels (Section~\ref{sec:protocol-types})
capture exactly the information that value-sequence systems abstract:
the deterministic sequence of values on each channel.  Our
compositional deadlock-freedom theorem
(Theorem~\ref{thm:deadlock-free}) ensures that the composed system
is well-defined, and the protocol conformance check ensures that
each process produces the correct value sequence.  The domain
weakening condition corresponds to our requirement that a
process decomposition step must preserve the external protocols
(Section~\ref{sec:refinement-steps}).

In practice, most well-designed CSP systems are L-deterministic:
each process's control flow determines which channel it next
communicates on without observing the state of other
channels~\cite{papadantonakis2002}.
The MiniMIPS (Section~\ref{sec:minimips}) and the Caltech 80C51
microcontroller are both L-deterministic.
Not all systems qualify: a system that guards on probe negation
(``\texttt{\~{}\#L}'') to distinguish empty from non-empty channels
is not L-deterministic and may not be slack elastic.

The original slack elasticity theorem requires that each channel be
accessed at most once per loop iteration---a restriction that excludes
programs where a shared resource (e.g., a floating-point unit) is
accessed from multiple program points.  Wen, Li, and
Manohar~\cite{wen2025} remove this restriction by introducing channel
replicas and deterministic control circuits, extending slack
elasticity to all L-deterministic CHP programs regardless of access
multiplicity.  This extends the domain of our pipeline insertion step
(Section~\ref{sec:refinement-steps}) to the general case.

Prakash~\cite{prakash2008} showed that the slack
matching problem (optimally assigning buffer depths to channels in a
slack-elastic system) is NP-complete, formulating it as a mixed
integer linear program.

In the protocol type framework, slack elasticity can be verified as
follows:
\begin{enumerate}
\item Check that each process is L-deterministic: its control flow
  uniquely determines the next channel action from the local state.
  (Sufficient condition.  A syntactic check that excludes probe
  negation and data-dependent channel selection suffices for common
  cases.)
...
categories are:
\begin{description}
\item[\texttt{int}] --- arbitrary-precision integer ($\mathbb{Z}$).
...
check can be done once per template type and cached.  For channels
with repeated access per iteration, the verifier must additionally
handle the channel-replica and control-circuit construction
of~\cite{wen2025}, checking that the replica network preserves the
original channel's protocol.
...
2. Protocol annotation & Matching & Syntactic & PASS \\
...
violation).  Deni\'elou and Yoshida~\cite{yoshida2012} showed
how multiparty session types correspond to communicating automata,
enabling automated verification of multiparty protocols.

Our protocol types are a specialization of binary session types to the
hardware domain: point-to-point channels with fixed-width channel
values and no delegation.  The key simplification is that our protocols are
...
processes, each directly implementable as a precharged half-buffer
(PCHB) pipeline stage.  DDD proceeds in three phases:
\begin{enumerate}
\item \textbf{DSA transformation.}  The sequential program is
  converted to \emph{dynamic single-assignment} form, where each
  variable is assigned at most once per loop iteration.  Nested loops
  are eliminated by early decomposition, and selection statements and
  straightline code are rewritten.  This removes false syntactic
  dependencies (analogous to WAW and WAR hazards in compiler
  terminology), exposing the true data parallelism.
\item \textbf{Projection.}  Variable dependency sets are
  computed: for each variable~$v$, the set $\mathit{DS}(v)$ contains
  all variables whose values are needed to compute~$v$.  When a
  variable appears in multiple dependency sets, copy variables and
  internal channels are inserted.  The DSA program is then
  syntactically partitioned so that each resulting process computes
  exactly one variable, communicating intermediate values over newly
  inserted channels.  The correctness of projection relies crucially
  on \emph{slack elasticity}: all deterministic CHP programs are
  locally slack elastic~\cite{manohar2001}, so adding channel buffers
  (slack) between projected processes cannot change the sequence of
  values communicated on any channel (Theorem~5 in~\cite{wong2004}).
\item \textbf{Clustering.}  The fine-grained system produced by
  projection typically contains many small processes with high
  communication overhead.  \emph{Distillation} makes certain
  communications conditional, allowing processes to go idle and
  consume no dynamic energy.  \emph{Elimination} removes
  trivial left--right buffer processes.  Finally,
  \emph{recomposition} merges selected processes to reduce channel
  count and energy, while \emph{slack matching} (using a
  forward-backward interval delay model and simulated annealing)
  inserts pipeline buffers to meet throughput targets.
\end{enumerate}
Wong proves that all processes generated by the DSA and projection
phases satisfy the strict CHP requirements for direct implementation
as asynchronous pipeline stages (no nested loops, at most one use of
each channel per iteration, all inputs precede all outputs).  DDD
applies only to deterministic CHP programs---connecting directly to
Papadantonakis's D-CHP restriction (Section~\ref{sec:dchp}).

The method was validated on the instruction-fetch unit of the
Lutonium asynchronous 8051 microcontroller~\cite{martin2003}: DDD
decomposed the sequential specification into 88~processes, reduced to
27~after distillation, elimination, and clustering.  The resulting
system matched the throughput of the manual decomposition (which had
required weeks of expert effort) at roughly $2{\times}$ the energy
cost---the price of full automation versus hand-tuned optimization.
...
The explicit dependence of DDD's projection on slack elasticity
underscores a recurring theme: the correctness of process
decomposition in asynchronous systems fundamentally requires that
adding buffering does not alter observable behavior.  Our protocol
types formalize exactly this invariant.

%----------------------------------------------------------------------
\subsection{Deterministic CHP and Value Sequence Systems}
\label{sec:dchp}
%----------------------------------------------------------------------

Papadantonakis~\cite{papadantonakis2002} introduced two concepts
that are foundational to our protocol type framework.

First, D-CHP (Deterministic CHP) restricts CHP by excluding probes
and the comma operator, yielding a language where all nondeterminism
is resolved by the environment (channel data values), never by
scheduling.  Within D-CHP, the hierarchy of determinism conditions
(L-deterministic, SS-deterministic, IO-deterministic) precisely
characterizes which systems are slack elastic: L-determinism is
sufficient, SS-determinism is not.  This gives our framework a
concrete syntactic criterion for identifying systems that tolerate
arbitrary slack assignment.

Second, and more fundamentally, Papadantonakis shows that slack
elasticity alone does not justify process decomposition.  He
introduces \emph{value sequence systems}---an abstraction where each
channel is characterized by the deterministic sequence of values it
carries---and defines \emph{domain weakening} as the formal basis for
decomposition transformations.  Our protocol types
(Section~\ref{sec:protocol-types}) are the syntactic realization of
value-sequence characterizations: each protocol annotation describes
the sequence of values on a channel, and protocol conformance ensures
that the process produces that sequence.  The channel dependency graph
(Definition~\ref{def:cdg}) captures exactly the inter-channel
dependencies that Papadantonakis's value-sequence functions express.
Domain weakening corresponds to our requirement that decomposition
preserves external protocols while introducing new internal channels
with their own value-sequence specifications.

Papadantonakis also developed m3-3, a Modula-3 message-passing
library for testing concurrent CHP systems, which is a precursor
to the simulation approach taken by \texttt{cspc}.
...
%----------------------------------------------------------------------
\subsection{General Slack Elasticity and Dataflow Translation}
%----------------------------------------------------------------------

Wen, Li, and Manohar~\cite{wen2025} extend the original slack
elasticity result~\cite{manohar2001} to handle CHP processes where a
channel is accessed \emph{multiple times per loop iteration}---a
common pattern when a shared hardware unit (e.g., a floating-point
multiply-accumulate) is accessed from multiple program points, or when
variable-length packet protocols require repeated reads on the same
channel.  Previous dataflow translation methods (DDD~\cite{wong2004}
and static token form) could not handle such programs without ad-hoc
manual rewriting.

The key idea is to introduce \emph{channel replicas}---one per access
site---and a structurally-inductive \emph{control circuit} (built from
sequential, conditional, and loop combinators) that dynamically
generates the merge/split control tokens needed to recombine the
replicas into the single original channel.  Because the construction
uses only the eight deterministic dataflow elements (never the
nondeterministic MIXER), the result is guaranteed to be slack elastic
by Corollary~2 of~\cite{manohar2001}, and is therefore amenable to
all standard pipeline optimizations (buffer insertion, slack matching,
retiming).

This work is directly relevant to our framework in two ways.  First,
it extends the domain of the ``pipeline insertion'' refinement step
(Section~\ref{sec:refinement-steps}) to processes with repeated
channel actions, which our current protocol types can describe but
which the original projection theorem could not decompose.  Second,
the determinism-preservation argument---that the construction
maintains program-order access to shared state, unlike MIXER-based
alternatives that can reorder accesses and produce erroneous
results---is precisely the kind of correctness property that our
protocol type checker should verify.

%----------------------------------------------------------------------
\subsection{Certified CHP Semantics}
%----------------------------------------------------------------------

Nkounkou~\cite{nkounkou2020} provides the first mechanically verified
formalization of CHP semantics, implemented in the Coq proof
assistant.  The formalization defines the behavior of a CHP program as
a set of possibly-infinite traces of event bags (multisets of atomic
events: assignments, channel send/receive phases, waits, and
determinism-violation markers).  A certified simulator, also
implemented and verified in Coq, is proved sound and complete with
respect to this trace semantics: every trace the simulator produces is
a valid feasible trace of the program, and every feasible trace can be
produced by some input sequence.

The formalization covers core CHP including sequential and parallel
composition, deterministic and nondeterministic selection, repetition,
channel send/receive with data, probes, and---notably---shared
variables across concurrent processes, which prior formalizations
excluded.  The simulator has been extracted to OCaml and demonstrated
on Martin's first asynchronous MIPS microprocessor (computing
Fibonacci numbers).

Nkounkou's work does not address deadlock-freedom, slack elasticity,
or refinement checking (all noted as future work), but it provides the
foundational semantic infrastructure on which such properties could be
mechanically verified.  For our framework, the most relevant
connection is the prospect of \emph{certified refinement checking}:
given a mechanically verified CHP semantics, one could in principle
prove that a decomposition step preserves trace equivalence with
machine-checked certainty, rather than relying on tool-checked
heuristics.  The gap between Nkounkou's current formalization and a
practical certified refinement checker is substantial (requiring
formalization of deadlock, fairness, and trace comparison), but the
semantic foundation is now in place.
...
\item \textbf{Protocol types on channels.}  Each channel carries an
  ...
  Protocol types are regular expressions over value sequences,
...
\textit{Proc.\ 4th International Conference on the Mathematics of
Program Construction}, Springer LNCS 1422, pp.\ 272--285, 1998.

\bibitem{lines1998}
A.\ Lines,
``Pipelined Asynchronous Circuits,''
M.S.\ thesis, California Institute of Technology, 1995
(Caltech CS-TR-95-21, revised 1998).
...
\bibitem{papadantonakis2002}
K.\ Papadantonakis,
``What is `Deterministic CHP', and is `Slack Elasticity' That
Useful?''
M.S.\ thesis, California Institute of Technology, 2002.

\bibitem{prakash2008}
P.\ Prakash,
\textit{Throughput Optimization of Quasi Delay Insensitive Circuits
via Slack Matching},
...
\bibitem{wen2025}
X.\ Wen, R.\ Li, and R.\ Manohar,
``Translating General Slack Elastic Programs into Dataflow Circuits,''
\textit{Proc.\ 29th IEEE International Symposium on Asynchronous
Circuits and Systems (ASYNC)}, pp.\ 80--88, 2025.

\bibitem{nkounkou2020}
B.R.\ Nkounkou,
\textit{Certified Software for Designing Asynchronous Circuits},
Ph.D.\ thesis, Cornell University, 2020.
