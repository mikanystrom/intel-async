...
CSP~\cite{hoare1985} and Martin's CHP~\cite{martin1990}.  A process
...
endpoints are dual, then the composed system is deadlock-free
...
communications on a channel from the perspective of one endpoint.
Protocol types are regular expressions over send and receive actions,
...
\[
\pi ::= \texttt{!}\tau \mid \texttt{?}\tau \mid
        \pi_1 \mathbin{;} \pi_2 \mid
        \pi_1 \mid \pi_2 \mid
        \pi^{*} \mid \texttt{end}
\]
where $\tau$ is a data type (possibly with constraints), $\mathbin{;}$
is sequencing, $\mid$ is choice, ${}^{*}$ is repetition, and
\texttt{end} is the terminated protocol.
\end{definition}
...
\begin{Verbatim}[samepage=true]
var c : channel(32) protocol { !x ; ?ack }*;
\end{Verbatim}

This declares a channel where the sender repeatedly sends a 32-bit
value and then receives an acknowledgment.  The protocol is from the
sender's perspective; the receiver sees the dual protocol
\texttt{\{ ?x ; !ack \}*}.

More examples:

\begin{Verbatim}[samepage=true]
// Simple data channel: sender sends one value per handshake
var data : channel(32) protocol { !v }*;

// Request-response: sender sends request, receives response
var rr : channel(32) protocol { !req ; ?resp }*;

// Conditional: branch on data value
var ctrl : channel(1) protocol { !b ;
    [ b=0 -> !payload32 ; ?ack
    | b=1 -> end ] }*;
\end{Verbatim}

\subsubsection{Protocol duality}

The dual of a protocol $\pi$, written $\overline{\pi}$, is obtained by
swapping every send with a receive and vice versa:

\begin{align*}
\overline{\texttt{!}\tau} &= \texttt{?}\tau \\
\overline{\texttt{?}\tau} &= \texttt{!}\tau \\
\overline{\pi_1 \mathbin{;} \pi_2} &=
  \overline{\pi_1} \mathbin{;} \overline{\pi_2} \\
\overline{\pi_1 \mid \pi_2} &=
  \overline{\pi_1} \mid \overline{\pi_2} \\
\overline{\pi^{*}} &= (\overline{\pi})^{*} \\
\overline{\texttt{end}} &= \texttt{end}
\end{align*}

Every channel must have dual protocols at its two endpoints: if the
sender's protocol is $\pi$, the receiver's protocol must be
$\overline{\pi}$.
...
Protocol types as defined here are closely related to \emph{binary
session types}~\cite{honda1998}.  The key correspondences are:

\begin{center}
\begin{tabular}{ll}
\toprule
\textbf{Session types} & \textbf{Protocol types} \\
\midrule
Output type $!\tau.\sigma$ & $\texttt{!}\tau \mathbin{;} \pi$ \\
Input type $?\tau.\sigma$ & $\texttt{?}\tau \mathbin{;} \pi$ \\
Selection $\oplus\{l_i: \sigma_i\}$ & Choice $\pi_1 \mid \pi_2$ \\
Branching $\&\{l_i: \sigma_i\}$ & Dual choice \\
...
The simplification relative to full session types is that our channels
are point-to-point and carry fixed-width data, which eliminates the
need for delegation (channel passing) and polymorphism.  This
simplification is precisely what makes the checking problem decidable
and efficient---protocol conformance reduces to language inclusion over
regular languages.
...
\item[(b)] Every channel $c_j$ has dual protocols at its two endpoints.
...
action on $c_{i_1}$ matches the protocol.  By duality~(b), the other
endpoint's process $Q$ expects the dual action.  By protocol
conformance of $Q$, $Q$ is ready for this action (since $Q$'s prior
...
\item $q_1, \ldots, q_r$ are new internal channels connecting $P_1$
  to $P_2$, with $\bar{q}_i$ being the dual endpoint of $q_i$.
\end{itemize}
\end{definition}
...
(including the new internal channels), the internal channels have dual
protocols, and the augmented channel dependency graph remains acyclic
...
inputs, and that the internal channel~$M$ has matching dual protocols
at its endpoints.
...
obligation is that the buffer conforms to both $\pi$ (on its input
port) and $\overline{\pi}$ (on its output port)---which is trivially
the case for a FIFO buffer.
...
*[[ count < 4 -> L?buf[tail] ; tail = (tail+1) % 4 ; count = count+1
  : count > 0 -> R!buf[head] ; head = (head+1) % 4 ; count = count-1
]]
\end{Verbatim}
...
\item Each port's \keyword{conforms} protocol matches the channel's
  \keyword{protocol} (or its dual, depending on direction).
...
\item \textbf{Protocol duality.} Each channel's sender protocol is the
  dual of its receiver protocol.
...
\item \textbf{Duality of new internal channels.}
  The \texttt{fd\_pc} channel has protocol \texttt{\{ !v \}*} at the
  Fetch end and \texttt{\{ ?v \}*} at the Decode end---these are dual.
...
to the concrete bit widths required for synthesis.

\subsubsection{Type narrowing}

The specification may use \texttt{int} (unbounded) or \texttt{int(32)}
throughout.  Data refinement introduces the minimal widths:

\begin{Verbatim}[samepage=true]
// Before: all 32-bit
var de_imm : channel(32);

// After: immediate is 16-bit (MIPS I-type)
var de_imm : channel(16);
\end{Verbatim}
...
\item \textbf{Width consistency.}  Every assignment, arithmetic
  operation, and channel communication is type-correct at the new
  widths.  No implicit truncation occurs unless the designer
...
\item \textbf{Protocol preservation.}  The protocol on each channel
  ...
  For protocols with data constraints, the tool must verify that the
  constraints are still satisfiable at the narrower width.

\item \textbf{Value-range analysis.}  Optional but recommended: the
  tool performs range analysis to verify that no computation exceeds
  the declared width.  This is a standard abstract interpretation
...
This packing is a data refinement from separate channels to a single
structure channel.  The proof obligation is that the packing and
unpacking are inverses: if the sender packs fields into the structure
and the receiver unpacks them, the values are preserved.
...
\item \textbf{Lightweight:} Check that $P_1$ and $P_2$ each conform
  to their port protocols, that internal channels have dual
...
Protocol duality:
  in_ch:  PASS (sender { !instr }* dual of receiver { ?instr }*)
  out_ch: PASS (sender { !result }* dual of receiver { ?result }*)
...
The tool verifies duality at each channel endpoint.
...
Protocol duality: ALL PASS (14 channels checked)
Dependency graph:
...
Slack insertion verification:
  dc_rd (slack 1 -> 2): PASS
    Buffer conforms to { ?v ; !v }* on both ports
...
and the dual protocol on the other endpoint is its rely (the
...
\item \textbf{Compositional deadlock-freedom.}  If every process
  conforms to its port protocols, every channel has dual protocols at
