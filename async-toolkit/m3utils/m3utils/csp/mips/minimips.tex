...
\caption{MiniMIPS~II block diagram.  Five pipeline stages (blue) connected by
  ...
  All 35~channels use slack\,=\,1 (one-place buffer
...
\begin{lstlisting}[style=csp,caption={Syscall wrappers in C (using GCC inline assembly)},
  language=C,morekeywords={void,register,int,asm,volatile}]
...
\begin{lstlisting}[style=csp,caption={Collatz sequence --- C source and MIPS disassembly (excerpt)},
  language=C,morekeywords={void,int,while,if,else,return}]
...
\begin{lstlisting}[style=csp,caption={Collatz disassembly (selected instructions, \texttt{-O1})},
  language={},morekeywords={}]
...
flows through the five pipeline stages and reactive processes.  In this
instance the simulator is single-threaded; throughput is bounded by the
per-event overhead of the channel implementation.
